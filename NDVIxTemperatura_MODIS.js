...
var temp_collection = ee.ImageCollection('MODIS/061/MOD11A2');

function scale_and_temp(image) {
  var temp = image.select('LST_Day_1km');
  var Temperatura = temp.multiply(0.02).rename('Temp_s_c');
  return image
...
function cor_ndvi(image) {
  ...
  //.set({ data: image.date().format('YYYY-MM-dd') });
  return image.addBands(Temp_cor);}
var ndvi_collection2 = ee.ImageCollection('MODIS/061/MOD13Q1').map(cor_ndvi);
...
var grafico = ui.Chart.image.series({
  ...
  .setOptions({
    title: 'Coleção Mensal com NDVI e Temperatura - Arapiraca(AL)',
    lineWidth: 1,
    pointSize: 2,
    series: {
    ...
    vAxes: {
      ...
      1: {title: 'Temperatura (°C)', gridlines: {color: 'none'},
...
var grafico = ui.Chart.image.series({
  ...
  .setOptions({
    title: 'Pearsons Correlaction for NDVI and Temperatura - Arapiraca(AL)',
    lineWidth: 1,
    pointSize: 2,
    hAxis: {
      gridlines:  {color: 'none'}
